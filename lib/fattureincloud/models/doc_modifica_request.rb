...
module FattureInCloud
  class DocModificaRequest
    # Identificativo univoco del documento da modificare (ha priorità sul token)
    attr_accessor :id

    # Identificativo permanente del documento (utilizzato per identificare il documento solo se manca il parametro \"id\", non viene sovrascritto)
    attr_accessor :token

    # Identificativo univoco del cliente (serve per creare un collegamento tra il documento e un cliente in anagrafica; se specificato e nullo, il documento viene scollegato da qualsiasi cliente; se mancante, viene fatto il collegamento con piva o cf) [solo con tipo!=\"ordforn\"]
    attr_accessor :id_cliente

    # Identificativo univoco del fornitore (serve per creare un collegamento tra il documento e un fornitore in anagrafica; se specificato e nullo, il documento viene scollegato da qualsiasi fornitore; se mancante, viene fatto il collegamento con piva o cf) [solo con tipo=\"ordforn\"]
    ...
    # [Solo se tipo!=ddt] Identificativo del template del documento [Se non specificato viene utilizzato quello di default]
    attr_accessor :id_template

    # [Solo se ddt=true] Identificativo del template del ddt [Se non specificato viene utilizzato quello di default]
    attr_accessor :ddt_id_template

    # [Solo se ftacc=true] Identificativo del template della fattura accompagnatoria [Se non specificato viene utilizzato quello di default]
    ...
    # [Solo per fatture e ndc elettroniche] Indica se il documento è nel formato FatturaPA; se \"true\", vengono presi in considerazione tutti i successivi campi con prefisso \"PA_\", con eventuali eccezioni (se non valorizzati, vengono utilizzati i valori di default); se non valorizzato, i parametri con prefisso \"PA_\" non vengono modificati
    attr_accessor :pa
    ...
    attr_accessor :extra_anagrafica
    ...
    def self.attribute_map
      {
        ...
        :'extra_anagrafica' => :'extra_anagrafica',
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'PA_pec')
        self.pa_pec = attributes[:'PA_pec']
      else
        self.pa_pec = ''
      end
      ...
      if attributes.has_key?(:'extra_anagrafica')
        self.extra_anagrafica = attributes[:'extra_anagrafica']
      end
    ...
    def hash
      [id, token, id_cliente, id_fornitore, nome, indirizzo_via, indirizzo_cap, indirizzo_citta, indirizzo_provincia, indirizzo_extra, paese, paese_iso, lingua, piva, cf, autocompila_anagrafica, salva_anagrafica, numero, valuta, valuta_cambio, prezzi_ivati, rivalsa, cassa, rit_acconto, imponibile_ritenuta, rit_altra, marca_bollo, oggetto_visibile, oggetto_interno, centro_ricavo, centro_costo, note, nascondi_scadenza, ddt, ftacc, id_template, ddt_id_template, ftacc_id_template, mostra_info_pagamento, metodo_pagamento, metodo_titolo_n, metodo_desc_n, mostra_totali, mostra_bottone_paypal, mostra_bottone_bonifico, mostra_bottone_notifica, lista_articoli, lista_pagamenti, ddt_numero, ddt_colli, ddt_peso, ddt_causale, ddt_luogo, ddt_trasportatore, ddt_annotazioni, pa, pa_tipo_cliente, pa_tipo, pa_numero, pa_cup, pa_cig, pa_codice, pa_pec, pa_esigibilita, pa_modalita_pagamento, pa_istituto_credito, pa_iban, pa_beneficiario, extra_anagrafica, split_payment].hash
    end
