...
module FattureInCloud
  class MailListaRequest
    ...
    # Restituisce solo mail che si trovano in questo stato
    attr_accessor :stato
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'filtro')
        self.filtro = attributes[:'filtro']
      else
        self.filtro = ''
      end
    ...
    def valid?
      stato_validator = EnumAttributeValidator.new('String', ['in_coda', 'in_corso', 'inviato', 'fallito'])
      return false unless stato_validator.valid?(@stato)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] stato Object to be assigned
    def stato=(stato)
      validator = EnumAttributeValidator.new('String', ['in_coda', 'in_corso', 'inviato', 'fallito'])
      unless validator.valid?(stato)
        fail ArgumentError, 'invalid value for "stato", must be one of #{validator.allowable_values}.'
      end
      @stato = stato
    end
    ...
    def hash
      [filtro, destinatario, mittente, stato, pagina].hash
    end
